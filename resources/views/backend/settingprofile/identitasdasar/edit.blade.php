...
@section('title', 'Setting Profile Perusahaan - Edit Identitas Dasar')
@section('page-title', 'Setting Profile Perusahaan')
@section('breadcrumb', 'Setting / Profile Perusahaan / Edit Identitas')
...
<div class="container-fluid py-4">
  ...
  <div class="card bg-dark border-0 shadow-lg rounded-4 p-4">
    <h5 class="text-white fw-semibold mb-4">
      <i class="fas fa-edit me-2"></i> Edit Identitas Dasar
    </h5>

    {{-- Form Section --}}
    <form action="{{ route('admin.settingprofile.identitas.update', $profile->id) }}" method="POST" enctype="multipart/form-data">
      @csrf
      @method('PUT')
      @include('backend.settingprofile.identitasdasar.form', ['profile' => $profile])

      {{-- Tombol Aksi --}}
      <div class="mt-4 d-flex justify-content-end gap-2">
        <button type="submit" class="btn btn-warning fw-semibold text-dark shadow-sm px-4">
          <i class="fas fa-save me-1"></i> Perbarui
        </button>
        <a href="{{ route('admin.settingprofile.identitas.index') }}" class="btn btn-outline-light fw-semibold shadow-sm px-4">
